...
def _dense_builder(
    ...
    if strategy_name == "fedprox":
        return fedprox_wrapper.create_fedprox_dense_model(
            input_dim, num_classes, batch_size, strategy_runtime.client_strategy
        )
    if strategy_name == "feddyn":
        return feddyn_wrapper.create_feddyn_dense_model(
            input_dim,
            num_classes,
            batch_size,
            strategy_runtime.client_strategy,
            client_id,
        )
...
def _gru_builder(
    ...
    if strategy_name == "fedprox":
        return fedprox_wrapper.FedProxModelWrapper(
            base, strategy_runtime.client_strategy
        )
    if strategy_name == "feddyn":
        return feddyn_wrapper.FedDynModelWrapper(
            base, strategy_runtime.client_strategy, client_id
        )
    return base
...
def create_model(
    ...
    if architecture_key not in MODEL_REGISTRY:
        raise ValueError(
            f"Unknown model architecture '{architecture}'. Available: {', '.join(sorted(MODEL_REGISTRY))}"
        )
